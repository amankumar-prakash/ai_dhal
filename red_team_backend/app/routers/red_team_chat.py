"""Red-team supervised chat worker routes — service token auth."""
...
class DecisionBody(BaseModel):
    decision: str = Field(pattern="^(approve|stop)$")
...
async def post_message(
    ...
    try:
        ...
        code = 409 if ("not running" in str(exc) or "already being processed" in str(exc)) else 400
...
@router.post("/sessions/{session_id}/tool-calls/{call_id}/decision")
async def decide_tool_call(
    ...
    _require_token(x_service_token)
    try:
        session = await sess.get_registry().decide_tool_call(
            session_id, call_id, body.decision, body.reason
        )
    except KeyError:
        raise HTTPException(status_code=404, detail="Session not found") from None
...
async def session_summary(
    ...
    _require_token(x_service_token)
    try:
        markdown = sess.get_registry().summary_markdown(session_id)
    except KeyError:
        raise HTTPException(status_code=404, detail="Session not found") from None
