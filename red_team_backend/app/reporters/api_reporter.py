"""Report results to api_service with one retry on 503."""
...
class ApiReporter:
    ...
    def headers(self) -> dict[str, str]:
        return {"X-Service-Token": self.settings.red_service_token}
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            for attempt in range(2):
                ...
                if resp.status_code == 503 and attempt == 0:
                    log.warning("API 503, retrying once")
                    continue
    ...
    async def post_attack_chain(self, payload: dict[str, Any]) -> Any:
        return await self._request("POST", "attack-chains", json=payload)
