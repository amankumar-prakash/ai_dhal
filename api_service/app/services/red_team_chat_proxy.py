"""Proxy red-team chat calls to red/blue workers."""
...
async def get_session(team: str, session_id: UUID, settings: Settings | None = None) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        r = await client.get(f"{base}{_PREFIX}/sessions/{session_id}", headers={"X-Service-Token": token})
    if r.status_code >= 400:
        raise HTTPException(status_code=r.status_code, detail=r.text)
    return r.json()
...
async def post_message(
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        r = await client.post(
            f"{base}{_PREFIX}/sessions/{session_id}/messages",
            headers={"X-Service-Token": token},
...
async def decide_tool_call(
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        r = await client.post(
            f"{base}{_PREFIX}/sessions/{session_id}/tool-calls/{call_id}/decision",
            headers={"X-Service-Token": token},
...
async def stop_session(team: str, session_id: UUID, settings: Settings | None = None) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        r = await client.post(
            f"{base}{_PREFIX}/sessions/{session_id}/stop",
            headers={"X-Service-Token": token},
        )
    if r.status_code >= 400:
        raise HTTPException(status_code=r.status_code, detail=r.text)
...
async def get_summary(team: str, session_id: UUID, settings: Settings | None = None) -> str:
    settings = settings or get_settings()
    base, token = _worker(team, settings)
    async with httpx.AsyncClient(timeout=30.0) as client:
        r = await client.get(
            f"{base}{_PREFIX}/sessions/{session_id}/summary",
            headers={"X-Service-Token": token},
        )
    if r.status_code >= 400:
        raise HTTPException(status_code=r.status_code, detail=r.text)
...
async def stream_events(
    ...
    async with httpx.AsyncClient(timeout=None) as client:
        async with client.stream(
            "GET",
            f"{base}{_PREFIX}/sessions/{session_id}/events",
