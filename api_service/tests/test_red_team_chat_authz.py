"""Red-team chat is open to any authenticated role."""
...
def client(monkeypatch):
    ...
    monkeypatch.setattr("app.services.red_team_chat_proxy.create_session", fake_create)
...
def test_any_role_can_create_red_team_chat_session(client, monkeypatch, role):
    uid = uuid4()
    _auth(monkeypatch, uid, role)
    r = client.post(
        "/api/v1/red-team-chat/sessions",
