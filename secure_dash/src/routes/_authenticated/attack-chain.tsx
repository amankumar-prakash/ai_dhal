...
export const Route = createFileRoute("/_authenticated/attack-chain")({
  head: () => ({
    meta: [
      { title: "Attack Chain — SecureDash" },
      {
        name: "description",
        content:
          "Kill-chain view from recon to exfiltration, with linked threat events and findings per stage.",
      },
      { property: "og:title", content: "Attack Chain — SecureDash" },
      {
        property: "og:description",
        content: "Kill-chain stages with linked threat events and validated findings.",
      },
...
function AttackChain() {
  useRealtime(["threat_events", "findings"]);
  const steps = useQuery(chainStepsQuery);
  const events = useQuery(eventsQuery);
  const findings = useQuery(findingsQuery);
  const [selected, setSelected] = useState<Stage | null>(STAGES[0]);

  const byStage = useMemo(() => {
    const map = new Map<Stage, typeof steps.data>();
    for (const stage of STAGES) {
      map.set(
        stage,
        (steps.data ?? []).filter((s) => s.stage === stage).sort((a, b) => a.sequence - b.sequence),
      );
    }
    return map;
  }, [steps.data]);
  ...
  if (steps.isError) {
    return <ErrorBanner message="Attack chain unavailable." onRetry={() => steps.refetch()} />;
  }
  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHeader
        title="Attack Chain"
        subtitle="Validated kill-chain path assembled from agent interactions."
      />
        ...
        <div className="flex flex-col items-stretch gap-3 xl:flex-row xl:items-center">
          {STAGES.map((stage, i) => {
            ...
            return (
              <div
                key={stage}
                className="flex flex-1 flex-col items-stretch xl:flex-row xl:items-center"
              >
                <button
                ...
                {i < STAGES.length - 1 && (
                  <div
                    className="flex shrink-0 items-center justify-center py-1 xl:px-1 xl:py-0"
                    aria-hidden
                  >
      ...
      {selected && (
        <Panel className="mt-6">
          ...
          {panelSteps.length === 0 ? (
            ...
            <ul>
              {panelSteps.map((step) => {
                const ev = (events.data ?? []).find((e) => e.id === step.threat_event_id);
                const fi = (findings.data ?? []).find((f) => f.id === step.finding_id);
                return (
                  <li
                    key={step.id}
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mono micro mt-1" style={{ color: "var(--text-muted)" }}>
                        {ev
                          ? `${ev.technique} · ${ev.source_ip} · ${relTime(ev.occurred_at)}`
                          : "no linked event"}
                        {fi ? ` · ${fi.cve ?? "finding"} (CVSS ${Number(fi.cvss).toFixed(1)})` : ""}
                      </div>
                    </div>
